...
from states import LowStates


@bot.message_handler(commands=["low"], state=LowStates.s_start)
def low(message):
    """
    ...
    bot.set_state(message.chat.id, LowStates.s_city, message.chat.id)

@bot.message_handler(state=LowStates.s_city, is_digit=False)
def get_city(message):
    ...
    try:
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            print(data["s_city"])
        bot.set_state(message.from_user.id, LowStates.s_date_from, message.chat.id)
    except IndexError as ex:
        ex, bot.send_message(message.chat.id, f"Что то пошло не так, попробуйте снова")

@bot.message_handler(state=LowStates.s_city, is_digit=True)
...
@bot.message_handler(state=LowStates.s_date_from)
def get_date_from(message):
    ...
    bot.set_state(message.from_user.id, LowStates.s_date_to, message.chat.id)

@bot.message_handler(state=LowStates.s_date_to)
def get_date_to(message):
    ...
    bot.set_state(message.from_user.id, LowStates.s_count_human, message.chat.id)
...
@bot.message_handler(state=LowStates.s_count_human, is_digit=True)
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        for i in result.json().get("data").get("propertySearch").get("properties"):  # Цены
            try:
                ...
                continue
        i = min_max_price.index(min(min_max_price))
        print(min_max_price)
        initialize_db()
        try:
            print("Запросы")
            req = Request.create(
                ...
                price=price, #int(min(min_max_price)),
